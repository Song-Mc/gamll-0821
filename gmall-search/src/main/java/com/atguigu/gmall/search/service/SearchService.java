...
public class SearchService {
    ...
    private SearchSourceBuilder buildDsl(SearchParamVo paramVo){
        ...
        // 终止价格
        Double priceTo = paramVo.getPrictTo();
        // 至少有一个不为空，才需要添加价格区间的过滤
        ...
        if (!CollectionUtils.isEmpty(props)){
            props.forEach(prop ->{
                ...
                if (attr != null && attr.length == 2){
                    ...
                    // 分割后第一位就是attrId
                    boolQuery.must(QueryBuilders.termQuery("searchAtts.attdId", attr[0]));

                    // 分割后的第二位：8G-12G
                    String[] attrValues = StringUtils.split(attr[1],"-");
                    boolQuery.must(QueryBuilders.termQuery("searchAtts.attrValue", attrValues));
